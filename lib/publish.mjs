...
const CLI_HELP = `Usage: zet publish <command>

Commands:
  ai     Generate zet-cli.md (AI agent documentation)
  ide    Generate TypeScript declarations for IDE support

Output paths are configurable in zet.config.mjs:
  zet.setAiPublishPath('docs/')       zet-cli.md → docs/zet-cli.md (default: root, .md/.mdc path = exact file)
  zet.setIdePublishPath('.types/')   .d.ts → .types/.zet-cli/index.d.ts (default: .zet-cli/)
`;
...
- \`zet.styledLine(msg)\` — Styled output with tags: \`<b>\` bold, \`<i>\` info/green, \`<w>\` warning/yellow, \`<e>\` error/red. Combine with bold: \`<ib>\`, \`<wb>\`, \`<eb>\`. Stdout.
...
- \`zet.setAiPublishPath(path)\` — Set output path for \`zet publish ai\` (relative to project root). If path ends with \`.md\` or \`.mdc\`, writes to that file directly; otherwise writes \`zet-cli.md\` inside the directory.
- \`zet.setIdePublishPath(path)\` — Set output path for \`zet publish ide\` (relative to project root)

## Groups

The names \`publish\` and \`init\` are reserved. Groups organize commands under a prefix:
...
const DTS_CONTENT = `declare module "zet-cli" {
  ...
  interface Zet {
    ...
    register(signature: string): CommandBuilder;
    setAiPublishPath(path: string): void;
    setIdePublishPath(path: string): void;
...
function handleAiRules(projectRoot, config) {
  ...
  if (config.aiPublishPath) {
    const resolved = join(projectRoot, config.aiPublishPath);
    if (isMarkdownFile(config.aiPublishPath)) {
      filePath = resolved;
...
function handleIdeSpecs(projectRoot, config) {
  ...
  if (config.idePublishPath) {
    dtsDir = join(projectRoot, config.idePublishPath, ".zet-cli");
  } else {
...
export async function handleCli(argv, projectRoot, config) {
  ...
  process.stderr.write(`zet: unknown publish command '${subcommand}'\n`);
