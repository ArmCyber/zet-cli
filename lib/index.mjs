...
// ---------------------------------------------------------------------------
// CLI config (set via zet.setAiRulesPath / zet.setIdeSpecsPath)
// ---------------------------------------------------------------------------
...
const zet = {
  ...
  // --- CLI config setters ---
  setAiRulesPath(path) {
    cliConfig.aiRulesPath = path;
  },

  setIdeSpecsPath(path) {
    cliConfig.ideSpecsPath = path;
  },
  ...
  async init() {
    ...
    if (firstArg === "cli") {
      const { handleCli } = await import("./cli.mjs");
