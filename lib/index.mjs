...
// ---------------------------------------------------------------------------
// CLI config (set via zet.setAiPublishPath / zet.setIdePublishPath)
// ---------------------------------------------------------------------------
...
const RESERVED_NAMES = new Set(["publish", "init"]);
...
function flattenParts(parts) {
  ...
  for (const part of parts) {
    if (typeof part === "function" && part._isZetTemplate) {
      throw new Error(
        "A template was passed to .command() without being called — did you forget to add ()?"
      );
    }
...
function formatGlobalHelp() {
  ...
  const lines = [];
  lines.push(`${yellow}Usage:${reset} zet ${dim}<command> [options]${reset}`);
  lines.push("");
  ...
  lines.push(`${dim}Run "zet <command> --help" for more information.${reset}`);
  return lines.join("\n");
}
...
function formatCommandHelp(cmd) {
  ...
  // Usage line
  let usage = `${yellow}Usage:${reset} zet `;
  ...
  if (sig.args.length > 0) {
    const argParts = sig.args.map((a) =>
      a.required ? `${dim}<${a.name}>${reset}` : `${dim}[${a.name}]${reset}`
    );
    usage += ` ${argParts.join(" ")}`;
  }
  if (sig.acceptsRest) usage += ` ${dim}[args...]${reset}`;
  if (sig.options.length > 0) usage += ` ${dim}[options]${reset}`;
  lines.push(usage);
...
const zet = {
  ...
  // --- CLI config setters ---
  setAiPublishPath(path) {
    cliConfig.aiPublishPath = path;
  },

  setIdePublishPath(path) {
  ...
  styledLine(msg) {
    const codes = { b: ["1", "22"], i: ["32", "0"], w: ["33", "0"], e: ["31", "0"] };
    const styled = msg.replace(/<([biwe]{1,2})>([\s\S]*?)<\/\1>/g, (_, tag, content) => {
      let open = "";
      let close = "";
      for (const ch of tag) {
        open += outAnsi(codes[ch][0]);
        close += outAnsi(codes[ch][1]);
      }
  ...
  async init() {
    ...
    if (firstArg === "publish") {
      const { handleCli } = await import("./publish.mjs");
      await handleCli(argv.slice(1), process.env.ZET_ROOT_DIR || process.cwd(), cliConfig);
      process.exit(0);
    }
...
export { parseSignature, parseArgv, Command, CommandBuilder, Group, CommandOutput, TemplateResult, RestPlaceholder, formatGlobalHelp, formatCommandHelp, formatUncaughtError, flattenParts, rootGroup, groups, cliConfig };
export default zet;
